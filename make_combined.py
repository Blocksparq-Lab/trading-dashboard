...
def fetch_verified_investing():
    ...
    response = client.chat.completions.create(
        model='gpt-4o-mini',
        messages=[{'role': 'user', 'content': 'Extract key trading setups. List: Tickers, entry levels, stops, targets, patterns, confidence. Transcript: ' + transcript[:6000]}],
        max_tokens=800
    )
...
def fetch_mitch_ray():
    ...
    response = client.chat.completions.create(
        model='gpt-4o-mini',
        messages=[{'role': 'user', 'content': 'Extract crypto setups. List: BTC/ETH/altcoins, levels, patterns, candlesticks, indicators, bias. Transcript: ' + normalized[:6000]}],
        max_tokens=800
    )
...
if __name__ == '__main__':
    ...
    if vi_error:
        print('VI Error:', vi_error)
    if mr_error:
        print('MR Error:', mr_error)
    ...
    with open(os.path.expanduser('~/trading-ai/latest_briefing.txt'), 'w') as f:
        f.write(final)
    print()
    print('Saved to ~/trading-ai/latest_briefing.txt')
"""

with open('combined_briefing.py', 'w') as f:
    f.write(content)

print('combined_briefing.py created successfully')
