...
# Backup
cp combined_briefing.py "combined_briefing_backup_$(date +%Y%m%d_%H%M).py" 2>/dev/null || echo "No existing script to backup"

# Get GitHub info
read -p "GitHub username: " USER
read -p "Repo name [trading-dashboard]: " REPO
REPO=${REPO:-trading-dashboard}

# Check token
if [ -z "$GITHUB_TOKEN" ]; then
    echo "⚠️  Please set GITHUB_TOKEN first:"
    echo "   export GITHUB_TOKEN='your_token_here'"
    exit 1
fi

echo "✅ Configuring for $USER/$REPO..."

# Create new script
cat > combined_briefing.py << PYTHONCODE
#!/usr/bin/env python3
...
def parse_analysis(text):
    ...
    for line in text.split('\\n'):
        if re.match(r'^[A-Z]{2,5}\\s*[-:]', line):
            ...
            if len(nums)>=3:
                data['equities'].append({'ticker':re.match(r'^([A-Z]{2,5})',line).group(1),'entry':f"${nums[0]}",'stop':f"${nums[1]}",'target':f"${nums[2]}",'bias':'Bullish' if float(nums[2].replace(',',''))>float(nums[0].replace(',','')) else 'Bearish'})
        if 'BTC' in line:
            nums=re.findall(r'\\$?([\\d,]+)',line)
            if len(nums)>=2: data['crypto'].append({'name':'BTC','support':f"${nums[0]}",'resist':f"${nums[1]}",'bias':'Neutral'})
    return data
...
cat > .github/workflows/deploy.yml << 'YAMLCODE'
...
2. Run: python3 combined_briefing.py
MDCODE
...
echo ""
echo "Next steps:"
echo "1. Create repo: https://github.com/new (name: $REPO, public)"
echo "2. Enable Pages: Settings → Pages → GitHub Actions"
echo "3. Commit and push:"
echo "   git add ."
echo "   git commit -m 'Phase 1'"
echo "   git push -u origin main"
echo "4. Run: python3 combined_briefing.py"
echo ""
echo "Dashboard will be at: https://$USER.github.io/$REPO/"
